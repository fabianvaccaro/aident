...
namespace ImProcessing
{
	...
	public sealed class ColorSpaceHelper
	{
        ...
        /// <summary>
        /// Converts RGB to Normalized RGB (nRGB)
        /// </summary>
        /// <param name="rgb"></param>
        /// <returns></returns>
        public static nRGB RGBtonRGB(RGB rgb)
        {
            double exponent = 2;
            double redD = Convert.ToDouble(rgb.Red);
            double greenD = Convert.ToDouble(rgb.Green);
            double blueD = Convert.ToDouble(rgb.Blue);

            double redResult = Math.Pow(redD, exponent);
            double greenResult = Math.Pow(greenD, exponent);
            double blueResult = Math.Pow(blueD, exponent);

            double totalResultado = redResult + greenResult + blueResult;
            totalResultado = Math.Sqrt(totalResultado);
            nRGB nrgb = new nRGB();
            nrgb.Red = redD / totalResultado;
            nrgb.Green = greenD / totalResultado;
            nrgb.Blue = blueD / totalResultado;
        ...
        /// </summary>
        public static LUV XYZtoLUV(double x, double y, double z)
        {
            LUV respuesta = new LUV();
            Double yn = 1.0;
            Double Lt = ((double)6 / (double)29);
            Lt = Lt * Lt * Lt;
            Double Up = 0.19784977571475;
            Double Vp = 0.46834507665248;
            Double l = 0;
            Double u = 0;
            Double v = 0;
            ...
            Double l0 = y / yn;
            l = l0;
            Double Potencia = (double)1 / 3;
            Double tr = System.Math.Pow(l0, Potencia);
            if(l>Lt)
            {
                l = 116 *tr  - 16;
            }
